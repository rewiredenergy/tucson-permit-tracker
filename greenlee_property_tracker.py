"""
Greenlee County Property Tracker
----------------------------------
Builds a per-parcel property profile for Greenlee County, Arizona (~4.7k
parcels, the smallest county in the state) from the Arizona Department of
Water Resources' (ADWR) statewide "Parcels_for_TEST" FeatureServer, layer 3
("Greenlee_Parcels") -- a public fallback service that hosts one parcel
layer per rural AZ county that doesn't run its own mature public-facing
GIS FeatureServer. Confirmed live via direct browser query: 4,706 parcels.
...
found for Greenlee, which does not run one.
...
Greenlee County includes portions of tribal/reservation land near its
borders; as with the other ADWR-sourced counties, only county-assessed
parcels appear in this layer.
...
Cochise/Navajo/Apache/Gila/Graham).
...
FEATURE_URL = ("https://azwatermaps.azwater.gov/arcgis/rest/services/General/"
               "Parcels_for_TEST/FeatureServer/3/query")
...
HEADERS = {
    "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) RewiredGreenleePropertyTracker/1.0",
...
# there. jurisdiction defaults to "greenlee_county" since the column is
# NOT NULL.
ROW_COLUMNS = [
...
def normalize_row(row: dict) -> dict:
    out = {"parcel": row["parcel"], "jurisdiction": row.get("jurisdiction") or "greenlee_county"}
...
def main() -> None:
    ...
    while True:
        ...
        upsert("greenlee_property_info", deduped, on_conflict="parcel")
