...
# Portal API helpers
# ---------------------------------------------------------------
def portal_post(path: str, payload: dict) -> dict:
...
def main() -> None:
    ...
    supabase_upsert(all_full_rows)   # rows must be upserted before events
    supabase_upsert(all_light_rows)  # (events reference permits by ID)
    supabase_insert_events(events)
    print("Done.")
