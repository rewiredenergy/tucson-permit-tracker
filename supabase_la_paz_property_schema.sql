-- La Paz County property/owner info table.
-- Source: ADWR statewide "Parcels_for_TEST" FeatureServer, layer 4
-- (La Paz County doesn't run its own public GIS FeatureServer). ~16,156
-- parcels, source extract dated May 2023. See la_paz_property_tracker.py
-- for details.
--
...
-- Navajo/Apache/Gila/Graham/Greenlee).

create table if not exists la_paz_property_info (
    parcel text primary key,  -- APN (falls back to the ADWR "ID" field if blank)
    jurisdiction text not null default 'la_paz_county',
...
create index if not exists la_paz_property_info_owner_name_idx
    on la_paz_property_info (owner_name);

create index if not exists la_paz_property_info_property_address_idx
    on la_paz_property_info (property_address);

create index if not exists la_paz_property_info_status_idx
    on la_paz_property_info (status);

alter table la_paz_property_info enable row level security;

create policy "Allow authenticated read access"
    on la_paz_property_info
    for select
    to authenticated
    using (true);
