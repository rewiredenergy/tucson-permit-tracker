"""
La Paz County Property Tracker
----------------------------------
Builds a per-parcel property profile for La Paz County, Arizona (~16.2k
parcels) from the Arizona Department of Water Resources' (ADWR) statewide
"Parcels_for_TEST" FeatureServer, layer 4 ("LaPaz_Parcels") -- a public
...
via direct browser query: 16,156 parcels, layer description states the
source extract is dated May 2023 ("U:\\Resources\\Data\\AZ_Parcels\\LaPaz\\2023\\May").
...
found for La Paz, which does not run one.
...
live against this exact layer, e.g. {"x": -114.528..., "y": 33.602...}
for parcel 30254039, correctly inside La Paz County (Ehrenberg/Parker area).
...
Cochise/Navajo/Apache/Gila/Graham/Greenlee).
...
FEATURE_URL = ("https://azwatermaps.azwater.gov/arcgis/rest/services/General/"
               "Parcels_for_TEST/FeatureServer/4/query")
...
HEADERS = {
    "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) RewiredLaPazPropertyTracker/1.0",
...
# there. jurisdiction defaults to "la_paz_county" since the column is
# NOT NULL.
ROW_COLUMNS = [
...
def normalize_row(row: dict) -> dict:
    out = {"parcel": row["parcel"], "jurisdiction": row.get("jurisdiction") or "la_paz_county"}
...
def main() -> None:
    ...
    while True:
        ...
        upsert("la_paz_property_info", deduped, on_conflict="parcel")
